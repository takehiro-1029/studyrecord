...
<main class="l-main">
    <div class="l-main-wrapper">
        ...
        <div class="p-studyrecord">
            ...
            <div class="p-studyrecord__content">
                <p class="p-studyrecord__content-title">記録数</p>
                <p class="p-studyrecord__content-num">{{ $count_tweet }}</p> 
            </div>
            <div class="p-studyrecord__tweet">
                <a href="https://twitter.com/intent/tweet?text=StudyRecord学習記録報告です%0D%0A%0D%0A期間%20:%20{{ $month }}%0D%0A時間%20:%20{{ $count_hours }}h%0D%0A記録数%20:%20{{ $count_tweet }}%0D%0Ahttps://studynrecord.com%0D%0A%0D%0A%23駆け出しエンジニアと繋がりたい" target="_blank"><img src="{{ asset('/img/top_twitterlogo.jpeg') }}" alt=""></a>
                <p>学習記録をツイート</p>
            </div>
    ...
    <div class="p-tweet">
       <div class="p-tweet__signup"><a href="{{route('save_record')}}" class="p-tweet__signup-btn">新規登録</a></div>
        @foreach ($viewdata as $key => $data)
            <div class="p-tweet__box js-scroll-clickout" data-id="{{$data['day']}}">
                ...
                <div class="p-tweet__box__btn">
                    <a href="{{route('edit_record', $data['id'])}}" class="p-tweet__box__btn-edit">編集</a>
                    <a href="{{route('delete_record', $data['id'])}}" class="p-tweet__box__btn-delete js-delete_alert">削除</a>  
                </div>
            </div>
        @endforeach
    </div>
